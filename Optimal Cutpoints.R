...
my_mean = aggregate(data$'PDL1 (H-score)', by=list(data$Group), mean) ; colnames(my_mean)=c("Group" , "mean")
my_CI = aggregate(data$'PDL1 (H-score)', by=list(data$Group) , FUN = function(x) t.test(x)$conf.int) ; colnames(my_CI)=c("Group" , "CI")
...
ggplot(data) + 
  ...
  scale_y_continuous(name = "PDL1 (H-score)") +
  geom_errorbar(data = my_info, aes(x = Group, y = CIdiff, ymin = mean - CIdiff, ymax = mean + CIdiff), color = "Grey", width = 0.2 , size=1) +
  theme_bw(base_size = 18) +
  geom_jitter(aes(x = data$Group, y = data$'PDL1 (H-score)', color= data$`Tumor Code`), width = 0.2, size = 4) +
...
my_mean = aggregate(data$'CD3 (cells per mm^2)', by=list(data$'Tumor'), mean) ; colnames(my_mean)=c("Group" , "mean")
my_CI = aggregate(data$'CD3 (cells per mm^2)', by=list(data$'Tumor') , FUN = function(x) t.test(x)$conf.int) ; colnames(my_CI)=c("Group" , "CI")
my_info = merge(my_mean, my_CI, by.x=1 , by.y=1)
my_info$CIdiff = ((my_CI$CI[,2] - my_CI$CI[,1])/2)
...
ggplot(data, aes(x=data$'Tumor', y=data$'PD1 (cells per mm^2)', color=Group)) +
  ...
  theme_bw(base_size = 18) +
  geom_jitter(aes(x = data$'Tumor', y = data$'PD1 (cells per mm^2)', color=Group), shape = 1, width = 0.3, size = 4) +
